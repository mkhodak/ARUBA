    ...
    def evaluate(self,
        ...
        for batch in _mini_batches(train_set, inner_batch_size, inner_iters, replacement):
            ...
            if self._pre_step_op:
                self.session.run(self._pre_step_op)
            self.session.run(minimize_op, feed_dict={input_ph: inputs, label_ph: labels})
        test_preds, test_losses = self._test_predictions(train_set, test_set, input_ph, label_ph, transductive, predictions)
        num_correct = sum([pred == sample[1] for pred, sample in zip(test_preds, test_set)])
        total_loss = sum(test_losses)
        self._full_state.import_variables(old_vars)
        return num_correct, total_loss

    def _test_predictions(self, train_set, test_set, input_ph, label_ph, 
            transductive,
            predictions):
        #if self._transductive:
        if transductive:
            inputs, labels = zip(*test_set)
            return self.session.run(predictions, feed_dict={input_ph: inputs, label_ph: labels})
        res = []
        los = []
        for test_sample in test_set:
            ...
            r, l = self.session.run(predictions, feed_dict={input_ph: inputs, label_ph: labels})
            res.append(r[-1])
            los.append(l[-1])
        return res, los
