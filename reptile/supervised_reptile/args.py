...
def argument_parser():
    ...
    parser.add_argument('--adagrad', help='use AdaGrad instead of Adam', action='store_true')
    parser.add_argument('--aruba', help='use ARUBA instead of Adam', action='store_true')
    parser.add_argument('--zeta', help='zeta parameter of ARUBA', default=1.0, type=float)
    parser.add_argument('--p', help='p parameter of ARUBA', default=1.0, type=float)
    parser.add_argument('--decay', help='decay past task/gradient information in ARUBA', default=0.0, type=float)
    parser.add_argument('--adaptive', help='adapt ARUBA rate when evaluating', default=0.0, type=float)
    parser.add_argument('--train-with-sgd', help='use SGD inner loop during meta-training', action='store_true')
    return parser
...
def model_kwargs(parsed_args):
    ...
    if parsed_args.sgd:
        res['optimizer'] = 'sgd'
    if parsed_args.adagrad:
        res['optimizer'] = 'adagrad'
    if parsed_args.aruba:
        res['optimizer'] = 'aruba'
        res['niters'] = parsed_args.inner_iters
        res['zeta'] = parsed_args.zeta
        res['p'] = parsed_args.p
        res['decay'] = parsed_args.decay
        if parsed_args.adaptive:
            res['adaptive'] = [parsed_args.adaptive]
        res['train_with_sgd'] = parsed_args.train_with_sgd
    return res
